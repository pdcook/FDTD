...
# double slit geometry, not the same as in the paper
...
# mask for the double slit geometry
mask = np.zeros((Nx,Ny),dtype=bool)
mask[Ny//2-thickness//2:Ny//2+thickness//2,:] = True
mask[Ny//2-thickness//2:Ny//2+thickness//2, \
    Nx//2-3*thickness//2:Nx//2-2*thickness//2] = False
mask[Ny//2-thickness//2:Ny//2+thickness//2, \
    Nx//2+2*thickness//2:Nx//2+3*thickness//2] = False
...
Ez, Dz, Hx, Hy = FDTD.FDTD2D(media, pulse, 500, PMLSize = 20)
E = FDTD.Energy(Ez,Hx,Hy)
FDTD.Animate(Ez,"Electric Field, $\\tilde{E}_z(x,y)$",cmin=-0.1,cmax=0.1)
